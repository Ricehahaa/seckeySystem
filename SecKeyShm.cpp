...
int SecKeyShm::shmWrite(NodeSHMInfo * pNodeInfo)
{
	...
	for (int i = 0; i < m_maxNode; i++)
	{
			...
			strcmp(pNode->serverID, pNodeInfo->serverID) == 0)
		{
			...
			printf("写入共享内存成功\n");
			cout<<"xierushi:"<<pNodeInfo->serverID<<"    "<<pNodeInfo->clientID<<endl;
			cout<<"hehe"<<pNode->serverID<<"    "<<pNode->clientID<<endl;
			unmapShm();
			return 0;
		}
	}
...
int SecKeyShm::shmDelete(const char* clientID, const char* serverID)
{
	...
	NodeSHMInfo *pNode = static_cast<NodeSHMInfo*>(mapShm());
	...
	for(i=0; i<m_maxNode; i++)
	{
		...
		{
			cout<<pNode[i].serverID<<endl;
			cout<<pNode[i].clientID<<endl;
			//清空这个位置
